...
df_fig3 <- df_selected %>%
  ...
  pivot_longer(
    cols = -l10_uAs,
    names_to = "wAs Standard",
    values_to = "Status")

# (Label Standard)
df_fig3 <- df_fig3 %>%
  mutate(`wAs Standard` = factor(`wAs Standard`, 
    levels = c("wAs1","wAs10","wAs50"),
...
(fig3 <- df_fig3 %>%
  na.omit() %>%
  ggplot(aes(x = l10_uAs, fill = fct_rev(Status))) +
  geom_density(alpha = 0.4) +
  facet_wrap(. ~ fct_rev(`wAs Standard`)) +
